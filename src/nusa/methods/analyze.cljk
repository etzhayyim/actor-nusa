(ns nusa.methods.analyze
  ...
  CONSTITUTIONAL framing:
    G1 — every :hemp/* cultivar MUST be :thc-class :fiber | :low-thc. Any other class
         is REJECTED (raises ex-info); the data
...
;; preserving the same serialized representation.
...
;; insertion order (Counter increment order in screen-thc).

(defn- ordered-assoc
  "assoc k→v on a map carrying ::order metadata (first-touch key order, mirroring a
  map). array-map only preserves order ≤8 keys, so we track order explicitly."
  [m k v]
...
(defn analyze
  "Returns the heritage analysis map."
...
(defn- pybool
  "Boolean labels use True/False for report compatibility."
  [b] (if b "True" "False"))
